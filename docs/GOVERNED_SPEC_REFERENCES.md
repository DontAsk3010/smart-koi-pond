...
- Active handbook: `00_SMART_KOI_POND_CLOSED_LOOP_CONTROL_SYSTEM_HANDBOOK_ACTIVE.docx` — Drive ID `1m8uokr6rrEXJp_jtMfte7oQWaUDxJgwn` — current governed version `V0.11`
...
Implementation changes that alter safety, operating modes, system semantics, state ownership, validation, deployment, persistence, or acceptance criteria must update the governed Drive authority first or in the same governed change.
