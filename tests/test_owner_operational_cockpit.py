...
def test_cockpit_quick_controls_use_existing_governed_commands() -> None:
    script = OWNER_OPERATIONAL_COCKPIT_SCRIPT
    assert "sendCommand('inject_actuator_fault',{asset_id:'main_pump',mode:'failed_off'},'engineering')" in script
    assert "sendCommand('clear_actuator_fault',{asset_id:'main_pump'},'engineering')" in script
    assert "sendCommand('start_blackout'" in script
    assert "sendCommand('restore_power',{},'operator')" in script
    assert "sendCommand('inject_sensor_fault',{sensor_id:'do',mode:'dropout'},'engineering')" in script
...
def test_owner_navigation_leads_with_overview_cockpit() -> None:
    ...
    order = "const ORDER=['overview','process','trends','simulator','logic','events','integrated']"
