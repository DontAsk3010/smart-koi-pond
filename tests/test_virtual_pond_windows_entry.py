...
def test_owner_cockpit_is_primary_browser_surface() -> None:
    ...
    assert "Overview" in COMPOSED_INDEX_HTML
    assert "Pond Schematic" in COMPOSED_INDEX_HTML
    ...
    assert "processButton.click()" not in COMPOSED_INDEX_HTML
    assert "cockpit.id='ownerOperationalCockpit'" in COMPOSED_INDEX_HTML
