...
def _normal_source(*, qualified: bool) -> SourceWaterProfile:
...
def _service() -> tuple[Any, KoiStockRuntimeApplicationService]:
    ...
    # Establish a healthy, canonical life-support baseline without direct actuator writes.
    service.command(
...
def _nitrite_case(level: float, reason: str, suffix: str) -> dict[str, Any]:
    ...
    vstatuses = {str(_wire(task.status)) for task in verified.verification}
    incident_resolved = any(
        item.get("lifecycle") == "RESOLVED" and item.get("end_event_sequence") is not None
        for item in _incident_summary(recovered)
    )
    return _case(
        case_id=f"A_{suffix}",
        ...
        checks={
            ...
            "support_command_accepted": aerator is not None
            and aerator.accepted
            and aerator.final_on,
            "flow_support_command_accepted": pump is not None
            and pump.accepted
            and pump.final_on,
            "actuator_feedback_on": disturbed.feedback.get("backup_aerator") is not None
            and disturbed.feedback["backup_aerator"].feedback_on,
            "response_verification": "VERIFIED_SUCCESS" in vstatuses,
...
def _nitrate_case() -> dict[str, Any]:
    ...
    return _case(
        ...
        checks={
            ...
            "feeding_inhibited": feeder is not None
            and feeder.requested_on is False
            and feeder.accepted,
            ...
            "canonical_policy_verified": bool(
                disturbed.biology.get("feeding_plan", {}).get(
                    "water_quality_feed_inhibited", False
                )
            ),
            "incident_opened": "INCIDENT_OPENED" in codes,
            "incident_resolved": incident_resolved,
        ...
        evidence={
            ...
            "verification": {
                ...
                "feed_inhibited": disturbed.biology.get("feeding_plan", {}).get(
                    "water_quality_feed_inhibited"
                ),
                "forbidden_chemical_commands_present": sorted(
                    forbidden.intersection(disturbed.commands)
                ),
            },
            "recovery_classification": _wire(recovered.classification),
            "incidents": _incident_summary(recovered),
...
def _nh3_case() -> dict[str, Any]:
    ...
    service.command(
        ...
        {"profile": profile.to_dict(), "actor": "acceptance-matrix"},
        role="engineering",
    )
    start_sequence = runtime.events.events[-1].sequence
    service.command(
        "set_environment_state", {"parameter": "tan", "value": 0.5}, role="engineering"
    )
    service.command(
        "set_environment_state", {"parameter": "ph", "value": 7.5}, role="engineering"
    )
    low = service.command(
        "set_environment_state",
    ...
    service.command(
        "set_environment_state", {"parameter": "ph", "value": 8.5}, role="engineering"
    )
    ...
    verified = service.step(130.0)
    service.command(
        "set_environment_state", {"parameter": "tan", "value": 0.1}, role="engineering"
    )
    service.command(
        "set_environment_state", {"parameter": "ph", "value": 7.2}, role="engineering"
    )
    ...
    return _case(
        ...
        checks={
            ...
            "higher_risk_relation": high_nh3 is not None
            and low_nh3 is not None
            and high_nh3 > low_nh3,
            "canonical_derivation": derivation.get("formula_id")
            == "EPA_EMERSON_FRESHWATER_NH3_FRACTION_V1"
            and derivation.get("fabricated") is False,
            "high_risk_classified": any(
                reason in high.classification.reasons
                for reason in ("NH3_HIGH", "NH3_EMERGENCY")
            ),
...
def _ph_case(value: float, suffix: str) -> dict[str, Any]:
    ...
    start_sequence = runtime.events.events[-1].sequence
    disturbed = service.command(
        "set_environment_state", {"parameter": "ph", "value": value}, role="engineering"
    )
...
def _low_do_case() -> dict[str, Any]:
    ...
    start_sequence = runtime.events.events[-1].sequence
    disturbed = service.command(
        "set_environment_state", {"parameter": "do", "value": 3.8}, role="engineering"
    )
    verified = service.step(130.0)
    ...
    return _case(
        ...
        evidence={
            ...
            "response_after_130s": _parameter_evidence(
                verified, "dissolved_oxygen_mg_l"
            ),
            "verification": _verification(verified),
            "recovery_classification": _wire(recovered.classification),
            "incidents": _incident_summary(recovered),
...
def _pump_failure_case() -> dict[str, Any]:
    ...
    disturbed = service.step(1.0)
    verified = service.step(130.0)
    service.command(
        "clear_actuator_fault", {"asset_id": "main_pump"}, role="engineering"
    )
    recovered = service.step(1.0)
    recovered = service.step(1.0)
    ...
    return _case(
        ...
        checks={
            ...
            "low_flow_or_capability_reason": (
                "FLOW_LOW" in disturbed.classification.reasons
                or any("main_pump" in str(reason).lower() for reason in disturbed.classification.reasons)
                or disturbed.classification.state.value in {"DEGRADED", "FAILSAFE", "CORRECTING"}
            ),
            "backup_command_accepted": command is not None
            ...
            "flow_response_verified": "VERIFIED_SUCCESS" in vstatuses,
            "incident_opened": "INCIDENT_OPENED" in codes,
            "incident_resolved_after_repair": incident_resolved,
        },
        evidence={
            ...
            "flow_at_failure": _parameter_evidence(
                disturbed, "circulation_flow_l_min"
            ),
            "flow_after_130s": _parameter_evidence(
                verified, "circulation_flow_l_min"
            ),
            "verification": _verification(verified),
            "recovery_classification": _wire(recovered.classification),
            "incidents": _incident_summary(recovered),
...
def _source_water_unsafe_case() -> dict[str, Any]:
    ...
    service.command(
        ...
        {"profile": _normal_source(qualified=False).to_dict(), "actor": "acceptance-matrix"},
        role="engineering",
    )
    start_sequence = runtime.events.events[-1].sequence
    rejected = False
    ...
    return _case(
        ...
        evidence={
            "condition": _normal_source(qualified=False).qualification_snapshot(),
            "error": error,
...
def _source_water_safe_case() -> dict[str, Any]:
    ...
    service.command(
        "configure_source_water_profile",
        {"profile": _normal_source(qualified=True).to_dict(), "actor": "acceptance-matrix"},
        role="engineering",
    )
    start_sequence = runtime.events.events[-1].sequence
    ...
    exchange = completed.hydraulics.get("water_exchange", {})
    ...
    return _case(
        ...
        checks={
            ...
            "mixing_evidence_present": bool(last_exchange.get("parameter_results")),
            "phase_timeline_present": "OPERATING_MODE_PHASE_CHANGED" in codes,
        },
        evidence={
            "condition": _normal_source(qualified=True).qualification_snapshot(),
            "start": {
...
def _run_case(fn: Callable[[], dict[str, Any]], case_id: str, family: str, title: str) -> dict[str, Any]:
    try:
        return fn()
    except Exception as exc:  # evidence must fail honest, never fabricate PASS
        return {
            "case_id": case_id,
            "family": family,
            "title": title,
...
def matrix_payload() -> dict[str, Any]:
    cases = [
        ...
        _run_case(
            lambda: _ph_case(9.2, "HIGH"),
            "D_PH_HIGH",
            "D — pH HIGH / LOW",
            "pH HIGH",
        ),
        _run_case(
            lambda: _ph_case(5.8, "LOW"),
            "D_PH_LOW",
            "D — pH HIGH / LOW",
            "pH LOW",
        ),
...
def _case_card(case: dict[str, Any]) -> str:
    ...
    <section class="case {status.lower()}">
      <header><span class="badge">{status}</span><div><small>{html.escape(case['family'])}</small><h2>{html.escape(case['title'])}</h2></div></header>
      <ul>{checks}</ul>
